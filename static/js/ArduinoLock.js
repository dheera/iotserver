function ArduinoLock(device_id, label, state, div) {
  ...
  action  = $('<label class="property-locked mdl-switch mdl-js-switch mdl-js-ripple-effect" for="switch-' + device_id + '"><input type="checkbox" id="switch-' + device_id + '" class="mdl-switch__input" checked><span class="mdl-switch__label"></span></label>');
  action.find('input').bind('change', function() {
    deviceSet(device_id, 'locked', this.checked ? 1 : 0);
  });
  ...
  this.update = function(state) {
    ...
    if(ui.data('state')['locked']===1) {
      ui.find('.property-locked').get(0).MaterialSwitch.on();
    } else {
      ui.find('.property-locked').get(0).MaterialSwitch.off();
    }
  };
